...
// @input Component.Camera renderingCamera
...
// @input float imageSize = 1.0 {"label":"Size", "widget":"slider", "min":0.0, "max":2.0, "step":0.01}
...
function initialized() {
    ...
    if (checkProperties()) {
        ...
        configureImageTransform({
            ...
            renderingCamera: script.renderingCamera
        });
    }
}
...
function configureImageTransform(option) {
    ...
    offsets.setSize(aspectScaling.uniformScale(option.scaleToAdd * option.renderingCamera.aspect * scaleMultiplier));
...
function setupTracking() {
    ...
    var trackedImageMesh;
    var moveImageMesh;
    if (script.imageSort == 0) {
        trackedImageMesh = getImageComponent(script.trackedImageTransform);
        moveImageMesh = getImageComponent(script.movieImage);
...
function playAnimatedTexture() {
    var isAnimated = !!script.movie.control.duration;
    if (isAnimated) {
        ...
        var providerDuration = roundToNearest(provider.duration);
...
function findTrackingData() {
    var results = [];
    var scriptComponents = script.getSceneObject().getComponents("Component.ScriptComponent");

    for (var i = 0; i < scriptComponents.length; i++) {
        var component = scriptComponents[i];
        if (component && component.animData) {
            results.push(component.animData);
        }
    }
...
function checkProperties() {
    ...
    if (!script.renderingCamera) {
        showError("ERROR: Please assign the rendering camera (e.g. Orthographic camera) to the Rendering Camera input on FaceInVideoController script.");
        return false;
    }
